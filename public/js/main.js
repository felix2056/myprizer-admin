document.addEventListener("DOMContentLoaded", () => {
    ...
    if(quillss){
        ...
        var addPrizeForm = document.querySelector('#addPrizeForm');
        if(addPrizeForm){
            addPrizeForm.onsubmit = function() {
                event.preventDefault();

                // Populate hidden form on submit
                var description = document.querySelector('input[name=description]');
                description.value = JSON.stringify(quillss.innerHTML);
    ...
    if(tagsss){
        ...
        tagInput1.addData(['Cash Prizes' , 'Cars' , 'Bikes'])
    }
