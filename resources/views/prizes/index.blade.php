...
<div class="body-content px-8 py-8 bg-slate-100">
    <div class="flex justify-between mb-10">
        <div class="page-title">
            <h3 class="mb-0 text-[28px]">Prizes</h3>
    ...
    <div class="bg-white rounded-t-md rounded-b-md shadow-xs py-4">
        <div class="tp-search-box flex items-center justify-between px-8 py-8 flex-wrap">
            ...
            <div class="flex sm:justify-end sm:space-x-6 flex-wrap">
                <div class="search-select mr-3 flex items-center space-x-3 ">
                    <span class="text-tiny inline-block leading-none -translate-y-[2px]">Categories : </span>
                ...
                <div class="product-add-btn flex ">
                    <a href="{{ route('prizes.create') }}" class="tp-btn">Add Prize</a>
                </div>
            </div>
        </div>
        ...
        <div class="relative mx-8 mb-5">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-6 ">
                @foreach ($prizes as $prize)
                    @include('prizes._prize-card', ['prize' => $prize])
                @endforeach
            </div>
        </div>

        {{ $prizes->links('vendor.pagination.default') }}
...
@section('scripts')
    <script>
        ...
        document.getElementById('categories-select').addEventListener('change', function() {
            ...
            let prizes = document.querySelectorAll('.prizes-grid-item');
            
            prizes.forEach(function(prize) {
                let prizeCategory = prize.getAttribute('data-category');
                
                if (prizeCategory == category || category == 'all') {
                    prize.classList.remove('hidden');
                } else { prize.classList.add('hidden'); }
            });
        });
    </script>
@endsection
