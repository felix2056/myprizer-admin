<tr class="bg-white border-b border-gray6 last:border-0 text-start mx-9">
    ...
    <td class="px-3 py-3 text-end">
        <span class="text-[11px]  text-success px-3 py-1 rounded-md leading-none bg-success/10 font-medium text-end">{{ $user->role }}</span>
    </td>
    ...
    <td class="px-9 py-3 text-end">
        <div class="flex items-center justify-end space-x-2">
            <div class="relative" x-data="{ editTooltip: false }">
                <button onclick="window.open('{{ route('staff.edit', $user->id) }}')" class="w-10 h-10 leading-10 text-tiny bg-success text-white rounded-md hover:bg-green-600" x-on:mouseenter="editTooltip = true" x-on:mouseleave="editTooltip = false">
            ...
            <div class="relative" x-data="{ deleteTooltip: false }">
                ...
                <form id="delete-staff-{{ $user->id }}" action="{{ route('staff.destroy', $user) }}" method="POST">
                    @csrf
                    @method('DELETE')
                </form>
            </div>
